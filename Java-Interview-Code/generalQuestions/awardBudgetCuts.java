...
committee planned to give N research grants this year. However, due to spending cutbacks, 
...
that the least number of recipients is impacted and that the new budget constraint is 
met (i.e. sum of the N reallocated grants equals to newBudget).
...
public class awardBudgetCuts {
  ...
  static double findGrantsCap(double[] grantsArray, double newBudget) {
    //Approach 1
    // store all the values into a hashset
    //if set contains value smaller than even division of array then store in result array
    //remaining values erased and filled with size divided by cap sub values removed
      
    //Approach 2
    //sort the array from smallest to largest
    //store every value smaller than even split of cap into result array
    //replace remaining values with array length divided by cap sub values already in result array
  }
